...
func (r *GormProductMappingRepository) List(filter ProductMappingListFilter) ([]models.ProductMapping, int64, error) {
	...
	if filter.Search != "" {
		like := "%" + filter.Search + "%"
		q = q.Where(
			"product_mappings.local_product_id IN (SELECT id FROM products WHERE deleted_at IS NULL AND (title_json->>'zh-CN' ILIKE ? OR title_json->>'en-US' ILIKE ? OR title_json->>'zh-TW' ILIKE ?))",
			like, like, like,
		)
	}
